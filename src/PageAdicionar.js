...
import { getProdutos, postProduto } from "./api"

export default function PageAdicionar({setPage}){
    const [nome,setNome]=useState('')
    const [preco,setPreco]=useState('')
    const [erro,setErro]=useState('')
    const [acerto,setAcerto]=useState('')
    function postarProduto(){
        const promise=postProduto({nome,preco})
        promise.then(res=>{
            setAcerto('Produto adicionado')
        ...
        <Tela>
            ...
            <Caixa>
                ...
                <input 
                    ...
                    placeholder="Preço..."
                />
                <Back onClick={postarProduto}>Postar</Back>
            </Caixa>
            <h6>{erro}</h6>
